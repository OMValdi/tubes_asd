...
void editInvestor(List_child& LC);
...
void editInvestor(List_child& LC){
    system("cls");
    cout<<"...<< INVESTOR << EDIT DATA INVESTOR \n";
    cout<<"=============================== \n";
    cout<<"Masukkan nama investor : ";
    string jawab;
    string cariOrang;
    cin>>cariOrang;
    address_child P = searchChild(LC,cariOrang);
    if (P!=NULL){
        cout<<endl;
        cout<<"Ubah budget? [Y/N] : ";
        cin>>jawab;
        if ((jawab=="Y")||(jawab=="y")){
            cout<<"Masukkan budget : ";
            cin>>info(P).budget;
        }
        cout<<"Budget Telah di ubah";
        _getch();
        return;
}
}
...
void delInvestasi(List_parent LP,List_child LC,list_relasi& LR){
    ...
    cout<<"Masukkan nama investor : ";
    cin>>nama;
    P = searchChild(LC,nama);
    cout<<"Masukkan nama Pohon : ";
    cin>>namaPohon;
    Q = searchParent(LP,namaPohon);
    if (P!=NULL){
        cout<<"DATA POHON YANG DICARI : \n";
            cout<<"Nama pohon : "<<info(Q).namaPohon<<endl;
            cout<<"Harga pohon : "<<info(Q).biayaPohon<<endl;
            cout<<"Keuntungan (ROI) "<<info(Q).profitPohon<<"%\n";
            cout<<"Kerugian (LOSS) "<<info(Q).lossPohon<<"%\n";
            cout<<"Stock pohon : "<<info(Q).stockPohon<<endl;
            cout<<"================================================= \n";
        address_relasi R = first(LR);
        do{
           if((info(child(R)).namaOrang==nama)&&(info(Q).namaPohon == namaPohon)){
                periksa = true;
           } R = next(R);
        }while((R!=NULL)&&(periksa==false));
        if (periksa==false){
            deleteChild(LC,P,nama);
            deleteParent(LP,Q, namaPohon);
            cout<<"Data pohon berhasil dihapus";
            dealokasi(R);
            dealokasi(Q);
            dealokasi(T);
            ...
            cout<<"Data investor tidak bisa dihapus...";
            getch();
        }
    } else {
        cout<<"Data investor tidak ada...";
        getch();
    }
}
